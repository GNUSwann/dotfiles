...
function getDirectoryContents(directory)
   local handle = io.popen("ls -Ap " .. directory)
   ...
   for entry in output:gmatch("[^\r\n]+") do
      ...
      local isDirectory = path:sub(-1) == "/"
      table.insert(contents, { type = isDirectory and "directory" or "file", name = entry })
   end
   return contents
end
...
local endpoints = {
   ...
   ["api/dir"] = {
      GET = function(request)
         local t = ""
         for i, dir in pairs(request.params) do
            t = t .. "/" .. dir
         end
         t = t .. "/"
         if t == "/" then
         ...
         local json = getDirectoryContents(t)
         if not json then
            return response(503, "plain", "Error: Not ready to handle requests.", {})
         end
         local rep = utils.format_json({ pwd = t, ls = json })
         return response(200, "json", rep, {})
      end,
   },
   ["api/toggle_controls"] = {
      POST = function(_)
         if toggle_controls then
         ...
         toggle_controls = not toggle_controls
         return handle_post(1, nil)
      end,
   },
   ["api/play"] = {
      POST = function(_)
